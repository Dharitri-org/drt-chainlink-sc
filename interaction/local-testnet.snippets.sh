...
METACHAIN_ADDRESS="moa1qqqqqqqqqqqqqqqpqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqzllls29jpxv"
...
address_of() {
    ...
    (set -x; moapy wallet pem-address $WALLET)
}

hex_address_of() {
    local WALLET=$(wallet_of $1)
    (set -x; moapy wallet pem-address-hex $WALLET)
}

bech_to_hex() {
    (set -x; moapy wallet bech32 --decode $1)
}

account_of() {
    local ADDRESS=$(hex_address_of $1)
    (set -x; moapy account get --address $ADDRESS)
}

balance_of() {
    local ADDRESS=$(hex_address_of $1)
    (set -x; moapy account get --balance --address $ADDRESS)
}

load_var() {
    local VAR_NAME=$1
    local VAR_VALUE="$(moapy data load --key=${VAR_NAME})"
...
save_var() {
    ...
    local VAR_VALUE=$2
    export -n "${VAR_NAME}=$VAR_VALUE"
    moapy data store --key=${VAR_NAME} --value=${VAR_VALUE}
}
...
deploy_contract() {
    ...
    (set -x; moapy --verbose contract deploy --bytecode="${CONTRACT_NAME_LOWERCASE}/output/${CONTRACT_NAME_LOWERCASE}.wasm" --recall-nonce \
...
build_contract() {
    CONTRACT_NAME=$1
    (set -x; moapy --verbose contract build "${CONTRACT_NAME}")
}
...
call_sc_address() {
    ...
    (set -x; moapy --verbose contract call ${CONTRACT_ADDRESS} --recall-nonce --pem=${WALLET} \
...
issue_tokens() {
    ...
    (set -x; moapy --verbose tx new ${CONTRACT_ADDRESS} --recall-nonce --pem=${OWNER_WALLET} \
...
send_moax() {
    ...
    (set -x; moapy --verbose tx new $OTHER_ARGUMENTS --recall-nonce --pem=${OWNER_WALLET} --gas-limit=100000 --send)
}
...
send_dct() {
    ...
    (set -x; moapy --verbose tx new --recall-nonce --pem=${SOURCE_WALLET} --receiver $(address_of $DESTINATION) \
        --gas-limit=400000 --data "DCTTransfer@$TOKEN_ID_HEX@$AMOUNT_HEX" --send)
}
...
send_dct_with_call() {
    ...
    local TOKEN_ID_HEX=$(ascii_to_hex $TOKEN_ID)
    local AMOUNT_HEX=$(number_to_hex $AMOUNT)
    (set -x; moapy --verbose tx new --recall-nonce --pem=${SOURCE_WALLET} --receiver $DESTINATION_ADDRESS \
        --gas-limit=500000000 --data "DCTTransfer@$TOKEN_ID_HEX@$AMOUNT_HEX@$OTHER_ARGUMENTS" --send)
}

check_issued_tokens() {
    call $METACHAIN_ADDRESS eve getAllDCTTokens 0
}
...
step_1_issue_tokens() {
    ...
    sleep 6
    call_sc_address $METACHAIN_ADDRESS eve --function getAllDCTTokens
}

# note: token names can be obtained from the results of the getAllDCTTokens call from step 1
...
step_5_prepare_exchange() {
    send_dct_with_call eve ${EXCHANGE_EVE_ADDRESS} $TOKEN_A $SOME_TOKENS "$(ascii_to_hex deposit)"
    
    sleep 6

    send_dct_with_call eve ${EXCHANGE_EVE_ADDRESS} $TOKEN_B $SOME_TOKENS "$(ascii_to_hex deposit)"
}

step_6_send_funds_to_other_users() {
    send_dct eve heidi $TOKEN_A $SOME_TOKENS
    
    sleep 6

    send_dct eve ivan $TOKEN_B $SOME_TOKENS
}
...
exchange_tokens() {
    ...
    send_dct_with_call $OWNER $EXCHANGE_EVE_ADDRESS $TOKEN $AMOUNT \
