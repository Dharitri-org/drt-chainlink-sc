...
pub trait MoaxDctExchange {
    ...
    #[payable("*")]
    #[endpoint(exchange)]
    fn exchange(&self, target_token: MoaxOrDctTokenIdentifier) {
        let (source_token, payment) = self.call_value().moax_or_single_fungible_dct();
        require!(payment > 0, "Payment must be more than 0");
        require!(
            self.balance().contains_key(&source_token),
            "DCT provided as payment not supported by the exchange"
        );
        require!(
            self.balance().contains_key(&target_token),
            "Target DCT token not supported by the exchange"
        );
    ...
    fn check_aggregator_tokens(
        ...
        source_token: &MoaxOrDctTokenIdentifier,
        target_token: &MoaxOrDctTokenIdentifier,
        ...
        let first_token = MoaxOrDctTokenIdentifier::parse(ManagedBuffer::new_from_bytes(first));
        let second_token = MoaxOrDctTokenIdentifier::parse(ManagedBuffer::new_from_bytes(second));
    ...
    fn increase_balance(&self, token_identifier: &MoaxOrDctTokenIdentifier, amount: &BigUint) {
    ...
    fn decrease_balance(&self, token_identifier: &MoaxOrDctTokenIdentifier, amount: &BigUint) {
        let mut balance = self
            .balance()
            .get(&token_identifier)
            .unwrap_or_else(|| BigUint::zero());
    ...
    fn balance(&self) -> MapMapper<MoaxOrDctTokenIdentifier, BigUint>;
